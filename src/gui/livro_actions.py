...
class LivroActions:
    ...
    def __init__(self, master, lista_livros, livro_manager):
        ...
        self.botao_exibir = tk.Button(frame_botoes, text='Abrir Link', command=self.abrir_link)
    ...
    def editar_livro(self):
        ...
        novo_link = simpledialog.askstring("Editar Livro", "Novo link:", initialvalue=getattr(livro, 'link', ''))
        ...
        if isinstance(livro, LivroFisico):
            ...
            livro_editado = LivroFisico(novo_titulo, novo_autor, localizacao=nova_localizacao, link=novo_link)
        else:
            novo_tamanho = simpledialog.askfloat("Editar Livro", "Novo tamanho (MB):", initialvalue=livro.tamanho)
            livro_editado = LivroDigital(novo_titulo, novo_autor, formato="PDF", tamanho=novo_tamanho, link=novo_link)
    ...
    def abrir_link(self):
        ...
        if hasattr(livro, "link") and livro.link and livro.link != "#":
            ...
            messagebox.showwarning("Aviso", "O livro selecionado não possui um link válido.")